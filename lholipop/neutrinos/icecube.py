# dummy implementation lifted from nu coindence
#
...
class IceCubeAlertsWrapper(IceCubeObsWrapper):
    ...
    def _simulation_setup(self):
        self._energy_res = EnergyResolution.from_dataset(
            "20150820", fetch=False
        )
    ...
    def _hese_simulation_setup(self):
        ...
        if self._parameter_server.hese.diffuse_flux is not None:
            diffuse_power_law = PowerLawFlux(
                **self._parameter_server.hese.diffuse_flux
            )
        ...
        self._hese_simulator.time = self._parameter_server.hese.detector[
            "obs_time"
        ]
        self._hese_simulator.max_cosz = self._parameter_server.hese.detector[
            "max_cosz"
        ]
    ...
    def _ehe_simulation_setup(self):
        ...
        if self._parameter_server.ehe.atmospheric_flux is not None:
            atmo_power_law = PowerLawFlux(
                **self._parameter_server.ehe.atmospheric_flux
            )
            atmo_source = DiffuseSource(flux_model=atmo_power_law)
        ...
        if self._parameter_server.ehe.diffuse_flux is not None:
            diffuse_power_law = PowerLawFlux(
                **self._parameter_server.ehe.diffuse_flux
            )
            diffuse_source = DiffuseSource(flux_model=diffuse_power_law)
        ...
        self._ehe_simulator.time = self._parameter_server.ehe.detector[
            "obs_time"
        ]
        self._ehe_simulator.max_cosz = self._parameter_server.ehe.detector[
            "max_cosz"
        ]
    ...
    def _run(self):
        ...
        if (
            ...
            hese_selection = (
                np.array(self._hese_simulator.reco_energy) > hese_Emin_det
            )
            ...
            ehe_selection = (
                np.array(self._ehe_simulator.reco_energy) > ehe_Emin_det
            )
            ...
            ra = np.concatenate(
                (self._hese_simulator.ra, self._ehe_simulator.ra)
            )
            ra = np.rad2deg(ra[selection])

            dec = np.concatenate(
                (self._hese_simulator.dec, self._ehe_simulator.dec)
            )
            ...
            energies = np.concatenate(
                (
                    self._hese_simulator.reco_energy,
                    self._ehe_simulator.reco_energy,
                )
            )
            ...
            source_labels = np.concatenate(
                (
                    self._hese_simulator.source_label,
                    self._ehe_simulator.source_label,
                )
            )
...
class IceCubeTracksWrapper(IceCubeObsWrapper):
    ...
    def _simulation_setup(self):
        ...
        if self._parameter_server.atmospheric_flux is not None:
            atmo_power_law = PowerLawFlux(
                **self._parameter_server.atmospheric_flux
            )
            atmo_source = DiffuseSource(flux_model=atmo_power_law)
        ...
        if self._parameter_server.diffuse_flux is not None:
            diffuse_power_law = PowerLawFlux(
                **self._parameter_server.diffuse_flux
            )
            diffuse_source = DiffuseSource(flux_model=diffuse_power_law)
...
def _get_point_source(
    ...
    energy_flux = luminosity / (
        4 * np.pi * cosmology.luminosity_distance(z) ** 2
    )
...
def _run_sim_for(
    ...
    tmp = PowerLawFlux(
        1, Enorm, spectral_index, lower_energy=Emin, upper_energy=Emax
    )
